app.controller("goodsController", function ($scope, $controller, $location, goodsService, uploadService,
    ...
    //选择了具体规格选项后保存到商品描述中；参数1：事件，参数2：规格名称，参数3：选项
    ...
    $scope.createItemList = function () {
        ...
        //对已经选择了的规格及其规格选项进行组装；遍历的内容形如：[{"attributeName":"网络","attributeValue":["移动3G","移动4G"]},{"attributeName":"机身内存","attributeValue":["16G","32G"]}]
        for (var i = 0; i < $scope.entity.goodsDesc.specificationItems.length; i++) {
            //spec的结构如：{"attributeName":"网络","attributeValue":["移动3G","移动4G"]}
            var spec = $scope.entity.goodsDesc.specificationItems[i];
            //对每个规格添加一个列，并且根据最新的结构，再根据规格选项设置记录
    ...
    //判断规格选项是否已经存在选择的里面；参数1：规格名称，参数2：选项
    $scope.checkAttributeValue = function (specName, optionName) {
        //当前商品所选择了保存在数据库中的规格及其选项；格式为：[{"attributeValue":["移动3G","移动4G"],"attributeName":"网络"},{"attributeValue":["16G","32G"],"attributeName":"机身内存"}]
        var items = $scope.entity.goodsDesc.specificationItems;
        //在上面的集合中查询attributeName为传入进来的规格名称的规格
